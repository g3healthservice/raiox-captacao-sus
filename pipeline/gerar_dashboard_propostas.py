...
<meta charset="UTF-8">
<script>/*G3ACCESSGATE v3 — controle de acesso: token por convite + login/senha permanente (sem expiração)*/(function(){var S="g3hs-2026-a7",T="rx";function d(s){s=s.replace(/-/g,"+").replace(/_/g,"/");while(s.length%4)s+="=";try{return decodeURIComponent(escape(atob(s)))}catch(e){return null}}function ck(p){var h=0,x=S+p,i;for(i=0;i<x.length;i++){h=((h<<5)-h+x.charCodeAt(i))|0}return(h>>>0).toString(36)}function tk(){var m=(location.hash+"&"+location.search).match(/[?#&]k=([^&]+)/);return m?decodeURIComponent(m[1]):null}function pr(t){if(!t)return null;var a=t.split(".");if(a.length!==2)return null;if(ck(a[0])!==a[1])return null;var s=d(a[0]);if(!s)return null;try{return JSON.parse(s)}catch(e){return null}}function login(){document.documentElement.innerHTML='<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Acesso &mdash; G3 Health Service</title></head><body style="margin:0;font-family:Arial,Helvetica,sans-serif;background:#0f2233;color:#e8eef4;display:flex;min-height:100vh;align-items:center;justify-content:center"><div style="width:100%;max-width:380px;text-align:center;padding:34px"><div style="font-size:46px;margin-bottom:10px">&#128274;</div><h2 style="margin:0 0 6px;color:#fff;font-size:22px">Acesso ao painel</h2><p style="margin:0 0 20px;line-height:1.5;color:#b7c6d4;font-size:14px">Informe seu e-mail e a senha de acesso.</p><input id="g3e" type="email" placeholder="seu@email.com" autocomplete="username" style="box-sizing:border-box;width:100%;padding:12px 14px;margin-bottom:10px;border:1px solid #2b4256;border-radius:8px;background:#0b1b29;color:#e8eef4;font-size:15px"><input id="g3p" type="password" placeholder="Senha" autocomplete="current-password" style="box-sizing:border-box;width:100%;padding:12px 14px;margin-bottom:6px;border:1px solid #2b4256;border-radius:8px;background:#0b1b29;color:#e8eef4;font-size:15px"><div id="g3msg" style="min-height:18px;margin:2px 0 10px;color:#ff8a8a;font-size:13px"></div><button id="g3btn" style="width:100%;padding:12px;border:0;border-radius:8px;background:#1a5276;color:#fff;font-size:15px;font-weight:bold;cursor:pointer">Entrar</button><p style="margin-top:22px;font-size:13px;color:#9db2c4">G3 Health Service<br><a href="mailto:g3.healthservice@proton.me" style="color:#7fb2e5">g3.healthservice@proton.me</a> &middot; +55 61 99255-7690</p></div></body>';var msg=document.getElementById("g3msg");function go(){var em=(document.getElementById("g3e").value||"").trim(),pw=document.getElementById("g3p").value||"";if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(em)){msg.innerHTML="Informe um e-mail v&aacute;lido.";return}if(pw!==PW){msg.innerHTML="Senha incorreta.";return}try{localStorage.setItem(LK,em)}catch(e){}location.reload()}document.getElementById("g3btn").addEventListener("click",go);function kd(e){if(e.key==="Enter"){e.preventDefault();go()}}document.getElementById("g3e").addEventListener("keydown",kd);document.getElementById("g3p").addEventListener("keydown",kd);if(window.stop)window.stop()}var LK="g3rx_login",PW="g3saude",o=pr(tk()),n=Date.now(),ok=false;if(o&&(o.perm||(o.t===T&&o.exp&&n<=o.exp)))ok=true;if(!ok){try{if(localStorage.getItem(LK))ok=true}catch(e){}}if(!ok)login();})();</script><meta name="viewport" content="width=device-width, initial-scale=1.0">
