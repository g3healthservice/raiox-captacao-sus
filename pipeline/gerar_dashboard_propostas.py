...
<meta charset="UTF-8">
<script>/*G3ACCESSGATE v2 — controle de acesso por convite (token com validade e ferramenta)*/(function(){var S="g3hs-2026-a7",T="rx";function d(s){s=s.replace(/-/g,"+").replace(/_/g,"/");while(s.length%4)s+="=";try{return decodeURIComponent(escape(atob(s)))}catch(e){return null}}function ck(p){var h=0,x=S+p,i;for(i=0;i<x.length;i++){h=((h<<5)-h+x.charCodeAt(i))|0}return(h>>>0).toString(36)}function tk(){var m=(location.hash+"&"+location.search).match(/[?#&]k=([^&]+)/);return m?decodeURIComponent(m[1]):null}function pr(t){if(!t)return null;var a=t.split(".");if(a.length!==2)return null;if(ck(a[0])!==a[1])return null;var s=d(a[0]);if(!s)return null;try{return JSON.parse(s)}catch(e){return null}}function bl(ti,ms){document.documentElement.innerHTML='<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'+ti+'</title></head><body style="margin:0;font-family:Arial,Helvetica,sans-serif;background:#0f2233;color:#e8eef4;display:flex;min-height:100vh;align-items:center;justify-content:center"><div style="max-width:460px;text-align:center;padding:34px"><div style="font-size:46px;margin-bottom:14px">&#128274;</div><h2 style="margin:0 0 10px;color:#fff;font-size:23px">'+ti+'</h2><p style="line-height:1.6;color:#b7c6d4;font-size:15px">'+ms+'</p><p style="margin-top:24px;font-size:14px;color:#9db2c4">G3 Health Service<br><a href="mailto:g3.healthservice@proton.me" style="color:#7fb2e5">g3.healthservice@proton.me</a> &middot; +55 61 99255-7690</p></div></body>';if(window.stop)window.stop()}var R="Esta ferramenta &eacute; apresentada mediante convite. Solicite o seu acesso &agrave; G3 Health Service.",o=pr(tk()),n=Date.now();if(!o){bl("Acesso restrito",R)}else if(o.perm){}else if(o.t!==T){bl("Acesso restrito",R)}else if(!o.exp||n>o.exp){bl("Acesso encerrado","O per&iacute;odo de acesso a esta demonstra&ccedil;&atilde;o foi conclu&iacute;do. Para retomar o acesso, fale com a G3 Health Service.")}})();</script><meta name="viewport" content="width=device-width, initial-scale=1.0">
